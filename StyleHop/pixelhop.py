...
class Pixelhop(cwSaab):
    ...
    def transform(self, X, styletransfer=False, transferlayer=[0,1,2], args={}):
        X = super().transform(X, styletransfer=styletransfer, transferlayer=transferlayer, args=args)
        return self.concatArg['func'](X, self.concatArg)
    
    def invTransform(self, X):
        X = super().invTransform(X)
        return self.concatArg['invfunc'](X, self.concatArg)
