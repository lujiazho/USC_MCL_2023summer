...
class Saab():
    ...
    def transform(self, X):
        ...
        X = remove_mean(X, self.Mean0)
        # print("transform kernel", self.Kernels.shape)
        # convolve with DC and AC filters
        X = feat_transform(X, self.Kernels)
        # print("transform output", X.shape)
        return X
    
    def invTransform(self, X):
        assert (self.trained == True), "Must call fit first!"
        # print("invTransform input", X.shape)

        # print("invTransform", X.shape, self.Kernels.shape)
        # inverse convolution
        X = feat_transform(X, np.linalg.inv(self.Kernels))

        # inverse removing mean
        X = remove_mean(X, -self.Mean0)
        ...
        X = X.astype('float32')
        # print("invTransform output", X.shape)
        return X
